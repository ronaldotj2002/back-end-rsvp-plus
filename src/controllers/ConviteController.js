...
class ConviteController {
    ...
    static async listarConvitesAtivos(req, res) {
        console.info("Iniciando a lista de Convites ativos..")
        try {
            const convites = await ConviteService.buscarConvitesAtivos();
            return res.status(200).json(convites);
        } catch (err) {
            return res.status(500).json(err.message);
        }
...
console.log("MUDAR O STATUS...", pegarConvite.id)
        const body = { usado: true }
        const id  = pegarConvite.id;
